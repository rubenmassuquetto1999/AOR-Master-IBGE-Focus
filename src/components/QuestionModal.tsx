...
export default function QuestionModal({
  ...
  const optionExpRefs = useRef<(HTMLTextAreaElement | null)[]>([]);
  ...
  const [previewOptions, setPreviewOptions] = useState<{ [index: number]: boolean }>({});
              ...
              {options.map((opt, i) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700">
                          <button
                            type="button"
                            onClick={() => {
                              const el = optionInputRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "bold", opt, (newVal) => {
                                  const newO = [...options];
                                  newO[i] = newVal;
                                  setOptions(newO);
                                });
                          ...
                          <button
                            ...
                            onClick={() => {
                              const el = optionInputRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "underline", opt, (newVal) => {
                                  const newO = [...options];
                                  newO[i] = newVal;
                                  setOptions(newO);
                                });
                          ...
                          <button
                            ...
                            onClick={() => {
                              const el = optionInputRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "mark-yellow", opt, (newVal) => {
                                  const newO = [...options];
                                  newO[i] = newVal;
                                  setOptions(newO);
                                });
                          ...
                          <button
                            ...
                            onClick={() => {
                              const el = optionInputRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "strike", opt, (newVal) => {
                                  const newO = [...options];
                                  newO[i] = newVal;
                                  setOptions(newO);
                                });
                          ...
                          <button
                            ...
                            className={`p-1 px-1.5 rounded-lg text-[10px] font-semibold flex items-center gap-0.5 transition ${
                              previewOptions[i]
                                ? "bg-indigo-600 text-white shadow-2xs"
                                : "hover:bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300"
                            }`}
                            title="Alternar prévia da alternativa"
                          >
                            <Eye className="w-3 h-3" />
                            <span className="hidden sm:inline">
                              {previewOptions[i] ? "Ocultar" : "Prévia"}
                            </span>
                          </button>
                        </div>
                      </div>
                    ...
                    {previewOptions[i] && (
                      <div className="p-2.5 rounded-xl bg-indigo-50/50 dark:bg-slate-800/80 border border-indigo-100 dark:border-slate-700 animate-fade-in text-xs font-semibold text-gray-800 dark:text-gray-100 leading-relaxed">
                        <span className="text-[10px] uppercase font-bold text-indigo-500 block mb-0.5">
                          Prévia da Alternativa {numOptions === 2 ? (i === 0 ? "Certo" : "Errado") : letter}:
                        </span>
                    ...
                    <div className="relative">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] uppercase font-bold text-amber-700 dark:text-amber-400/90">
                        ...
                        <div className="flex items-center gap-0.5">
                          <button
                            type="button"
                            onClick={() => {
                              const el = optionExpRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "bold", explanations[i] || "", (newVal) => {
                            ...
                            title="Negrito na justificativa"
                            className="p-0.5 px-1 rounded text-[10px] text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 font-bold hover:bg-slate-100 dark:hover:bg-slate-700"
                          >
                            B
                          </button>
                          <button
                            ...
                            onClick={() => {
                              const el = optionExpRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "underline", explanations[i] || "", (newVal) => {
                                  const newE = [...explanations];
                                  newE[i] = newVal;
                                  setExplanations(newE);
                                });
                            ...
                            title="Sublinhado na justificativa"
                            className="p-0.5 px-1 rounded text-[10px] text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 underline hover:bg-slate-100 dark:hover:bg-slate-700"
                          >
                            U
                          </button>
                          <button
                            ...
                            onClick={() => {
                              const el = optionExpRefs.current[i];
                              if (el) {
                                insertFormatInInput(el, "mark-yellow", explanations[i] || "", (newVal) => {
                                  const newE = [...explanations];
                                  newE[i] = newVal;
                                  setExplanations(newE);
                                });
                            ...
                            title="Marca-texto na justificativa"
                            className="p-0.5 px-1 rounded text-[10px] text-amber-600 hover:text-amber-700 dark:hover:text-amber-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                          >
                            Marca
                          </button>
                        </div>
                      ...
                      <textarea
                        ref={(el) => (optionExpRefs.current[i] = el)}
                        rows={2}
